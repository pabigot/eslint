...
2. Configuration options passed on the command line using `--rule`, `--global`, or `--env`.
...
* [`no-native-reassign`](http://eslint.org/docs/rules/no-native-reassign) was previously part of `no-unused-vars`, but was split out because it didn't make sense as part of another rule. The `no-native-reassign` rule warns whenever you try to overwrite a read-only global variable.
